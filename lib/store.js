...
function memStore() {
  ...
  return {
    ...
    async userCreate(u) {
      ...
      const id = 'u' + (++uid);
      const rec = { id, email, prenom: u.prenom || '', nom: u.nom || '', whatsapp: u.whatsapp || '', prefix: u.prefix || '+225', points: u.points || 0, passwordHash: u.passwordHash || null, notifSeenAt: 0, createdAt: Date.now() };
...
function neonStore() {
  ...
  return {
    ...
    async userCreate(u) {
      await ensureSchema();
      const r = await sql`
        INSERT INTO users (email, prenom, nom, whatsapp, prefix, points, password_hash)
        VALUES (${String(u.email).toLowerCase()}, ${u.prenom || ''}, ${u.nom || ''}, ${u.whatsapp || ''}, ${u.prefix || '+225'}, ${u.points || 0}, ${u.passwordHash || null})
        RETURNING *`;
      return rowToUser(r[0]);
    },
