...
function memStore() {
  ...
  const certs = new Map();            // id -> {id, userId, courseId, name, at}
  let uid = 0, aid = 0;
  ...
  return {
    ...
    async enrollmentUpsert(userId, courseId, o = {}) {
      ...
      m.set(courseId, {
        paid: o.paid != null ? o.paid : (prev.paid || false), orderRef: o.orderRef || prev.orderRef || null,
        expiresAt: o.expiresAt !== undefined ? o.expiresAt : (prev.expiresAt || null), at: prev.at,
      });
    },
    async enrollmentsByUser(userId) {
      const m = enrollments.get(userId); if (!m) return [];
      return [...m.entries()].map(([courseId, v]) => ({ courseId, paid: v.paid, orderRef: v.orderRef, expiresAt: v.expiresAt || null, at: v.at }));
    },
    ...
    async certCreate(userId, courseId, name) {
      for (const c of certs.values()) if (c.userId === userId && c.courseId === courseId) return { ...c };
      const id = 'BOYIA-' + Math.random().toString(36).slice(2, 8).toUpperCase() + '-' + Math.random().toString(36).slice(2, 6).toUpperCase();
      const rec = { id, userId, courseId, name, at: Date.now() };
      certs.set(id, rec); return { ...rec };
    },
    async certGet(id) { const c = certs.get(String(id || '').toUpperCase()); return c ? { ...c } : null; },
    async certsByUser(userId) { return [...certs.values()].filter(c => c.userId === userId).map(c => ({ ...c })); },
...
function neonStore() {
  ...
  async function ensureSchema() {
    if (!schemaReady) schemaReady = (async () => {
      ...
      await sql`ALTER TABLE enrollments ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ`;
      await sql`
  ...
  return {
    ...
    async enrollmentUpsert(userId, courseId, o = {}) {
      await ensureSchema();
      const exp = o.expiresAt !== undefined && o.expiresAt !== null ? new Date(o.expiresAt).toISOString() : null;
      await sql`
        INSERT INTO enrollments (user_id, course_id, paid, order_ref, expires_at)
        VALUES (${userId}, ${courseId}, ${o.paid || false}, ${o.orderRef || null}, ${exp})
        ON CONFLICT (user_id, course_id) DO UPDATE SET
          paid = enrollments.paid OR EXCLUDED.paid,
          order_ref = COALESCE(EXCLUDED.order_ref, enrollments.order_ref),
          expires_at = COALESCE(EXCLUDED.expires_at, enrollments.expires_at)`;
    },
    async enrollmentsByUser(userId) {
      await ensureSchema();
      const r = await sql`SELECT course_id, paid, order_ref, expires_at, created_at FROM enrollments WHERE user_id = ${userId}`;
      return r.map(x => ({ courseId: x.course_id, paid: x.paid, orderRef: x.order_ref, expiresAt: x.expires_at ? new Date(x.expires_at).getTime() : null, at: new Date(x.created_at).getTime() }));
    },
    async progressAdd(userId, courseId, lessonKey) {
      await ensureSchema();
      const r = await sql`
    ...
    async certCreate(userId, courseId, name) {
      ...
      const id = 'BOYIA-' + Math.random().toString(36).slice(2, 8).toUpperCase() + '-' + Math.random().toString(36).slice(2, 6).toUpperCase();
      const r = await sql`
        INSERT INTO certificates (id, user_id, course_id, name) VALUES (${id}, ${userId}, ${courseId}, ${name})
        ON CONFLICT (user_id, course_id) DO UPDATE SET name = EXCLUDED.name
        RETURNING *`;
      const c = r[0];
      return { id: c.id, userId: c.user_id, courseId: c.course_id, name: c.name, at: new Date(c.created_at).getTime() };
    },
    async certGet(id) {
      await ensureSchema();
      const r = await sql`SELECT * FROM certificates WHERE id = ${String(id || '').toUpperCase()} LIMIT 1`;
      const c = r[0]; return c ? { id: c.id, userId: c.user_id, courseId: c.course_id, name: c.name, at: new Date(c.created_at).getTime() } : null;
    },
    async certsByUser(userId) {
      await ensureSchema();
      const r = await sql`SELECT * FROM certificates WHERE user_id = ${userId} ORDER BY created_at DESC`;
      return r.map(c => ({ id: c.id, userId: c.user_id, courseId: c.course_id, name: c.name, at: new Date(c.created_at).getTime() }));
    },
    /* ---- Annonces & notifications ---- */
