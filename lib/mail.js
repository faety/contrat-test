...
/* ---- Annonce de l'équipe envoyée aux membres (optionnel, depuis l'admin) ---- */
async function sendAnnouncement(email, { title, body }) {
  if (!email) return { sent: false, reason: 'pas-d-email' };
  const paras = String(body || '').trim().split(/\n{2,}/).map(p =>
    `<p style="margin:0 0 12px">${esc(p).replace(/\n/g, '<br>')}</p>`).join('');
  const html = layout(title || 'Une annonce de Boyia Institute 📣', `
    ${paras}
    ${btn(APP_URL, 'Ouvrir l\'application')}
    <p style="font-size:13px;color:#66708C">Tu reçois ce message parce que tu as un compte Boyia Institute.</p>
  `);
  const text = `${title ? title + '\n\n' : ''}${String(body || '').trim()}\n\n${APP_URL}\n\nBoyia Institute`;
  return send({ to: email, subject: title ? `📣 ${title}` : '📣 Une annonce de Boyia Institute', html, text });
}

module.exports = { isConfigured, send, sendReceipt, sendWelcome, sendSaleNotification, sendLoginCode, sendAnnouncement, _setTransport };
