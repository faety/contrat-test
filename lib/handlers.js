...
async function linkPaidToAccount(o) {
  try {
    ...
    /* Abonnement : ouvre (ou PROLONGE) une période d'accès datée au cours lié.
       La prolongation part de la fin d'accès actuelle si elle est dans le futur —
       renouveler en avance ne fait jamais perdre de jours. */
    const plan = wave.SUB_PLANS[o.courseId];
    if (plan) {
      const enr = await store.enrollmentsByUser(user.id);
      const cur = enr.find(e => e.courseId === plan.course);
      if (cur && cur.orderRef === o.ref) return;   // idempotent : cette commande a déjà crédité l'accès
      const base = Math.max(Date.now(), (cur && cur.expiresAt) || 0);
      await store.enrollmentUpsert(user.id, plan.course, { paid: true, orderRef: o.ref, expiresAt: base + plan.days * 86400e3 });
      return;
    }
    /* Le pack débloque chacun des cours inclus. */
    const ids = o.courseId === 'pack' ? wave.PACK_COURSES : [o.courseId];
    for (const cid of ids) await store.enrollmentUpsert(user.id, cid, { paid: true, orderRef: o.ref });
...
async function hydrate(user) {
  const [enrollments, progress, certificates] = await Promise.all([
    store.enrollmentsByUser(user.id), store.progressByUser(user.id), store.certsByUser(user.id),
  ]);
  return {
    ...
    certificates: certificates.map(c => ({ id: c.id, courseId: c.courseId, name: c.name, at: c.at })),
  };
}
...
async function meEnroll(header, input) {
  ...
  /* Pendant la période gratuite Echo (wave.ECHO_FREE), « echo » s'inscrit
     librement comme un cours gratuit — sinon, abonnement obligatoire. */
  if (wave.PRICES[courseId] || wave.SUB_PLANS[courseId] || (wave.SUB_COURSES[courseId] && !wave.ECHO_FREE)) return { status: 400, body: { error: 'cours-payant' } };
  /* Ne touche jamais à une inscription existante (un abonnement payé garde ses jours). */
  const cur = (await store.enrollmentsByUser(user.id)).find(e => e.courseId === courseId);
  if (!cur) await store.enrollmentUpsert(user.id, courseId, { paid: false });
  return { status: 200, body: await hydrate(user) };
}

/* Marque une leçon terminée (+10 points, une seule fois).
   Cas particulier Echo : la clé est une PARTIE de leçon (ex. l2p3), validée
   contre le référentiel ECHO_LESSONS pour refuser les clés inventées. */
function validLessonKey(courseId, lessonKey) {
  if (courseId === 'echo') {
    const m = /^l(\d+)p(\d+)$/.exec(lessonKey);
    if (!m) return false;
    const total = wave.ECHO_LESSONS['l' + m[1]];
    const part = parseInt(m[2], 10);
    return !!total && part >= 1 && part <= total;
  }
  return /^l\d+$/.test(lessonKey);
}
async function meProgress(header, input) {
  ...
  if (!wave.isCourse(courseId) || !validLessonKey(courseId, lessonKey)) return { status: 400, body: { error: 'invalide' } };
  const enr = await store.enrollmentsByUser(user.id);
...
/* Délivre le certificat de complétion d'un cours — la complétion est validée
   CÔTÉ SERVEUR (toutes les leçons dans `progress`), pas sur parole du client. */
async function meCertificate(header, input) {
  const user = await userFromAuth(header);
  if (!user) return { status: 401, body: { error: 'non-connecte' } };
  const courseId = String((input && input.courseId) || '');
  if (courseId === 'echo') return echoCertificate(user);
  const needed = wave.COURSE_LESSONS[courseId];
  if (!needed) return { status: 400, body: { error: 'cours-inconnu' } };
  const [enr, prog] = await Promise.all([store.enrollmentsByUser(user.id), store.progressByUser(user.id)]);
  if (!enr.some(e => e.courseId === courseId)) return { status: 403, body: { error: 'non-inscrit' } };
  const done = prog.filter(p => p.courseId === courseId).length;
  if (done < needed) return { status: 400, body: { error: 'cours-non-termine', done, needed } };
  const fullName = `${(user.prenom || '').trim()} ${(user.nom || '').trim()}`.trim() || user.email;
  const cert = await store.certCreate(user.id, courseId, fullName);
  return { status: 200, body: { certificate: { id: cert.id, courseId, name: cert.name, at: cert.at }, ...(await hydrate(user)) } };
}

/* Certificats Echo par paliers (Niveau 1 = 3 leçons, 2 = 6, 3 = 10) — acquis à VIE,
   même si l'abonnement expire ensuite. Une leçon compte quand TOUTES ses parties
   sont dans `progress`. Chaque palier atteint est émis (idempotent), on renvoie le plus haut. */
async function echoCertificate(user) {
  const [enr, prog] = await Promise.all([store.enrollmentsByUser(user.id), store.progressByUser(user.id)]);
  if (!enr.some(e => e.courseId === 'echo')) return { status: 403, body: { error: 'non-inscrit' } };
  const parts = new Set(prog.filter(p => p.courseId === 'echo').map(p => p.lessonKey));
  let lessonsDone = 0;
  for (const [lid, total] of Object.entries(wave.ECHO_LESSONS)) {
    let n = 0;
    for (let i = 1; i <= total; i++) if (parts.has(`${lid}p${i}`)) n++;
    if (n >= total) lessonsDone++;
  }
  const achieved = wave.ECHO_CERT_LEVELS.filter(l => lessonsDone >= l.lessons);
  if (achieved.length === 0) {
    return { status: 400, body: { error: 'palier-non-atteint', done: lessonsDone, needed: wave.ECHO_CERT_LEVELS[0].lessons } };
  }
  const fullName = `${(user.prenom || '').trim()} ${(user.nom || '').trim()}`.trim() || user.email;
  let cert = null;
  for (const l of achieved) cert = await store.certCreate(user.id, `echo-n${l.level}`, fullName);
  return { status: 200, body: { certificate: { id: cert.id, courseId: cert.courseId, name: cert.name, at: cert.at }, lessonsDone, ...(await hydrate(user)) } };
}

/* Vérification PUBLIQUE d'un certificat par son identifiant (URL partageable). */
async function certVerify(id) {
  const c = id ? await store.certGet(id) : null;
  if (!c) return { status: 404, body: { valid: false } };
  return {
    status: 200,
    body: { valid: true, id: c.id, name: c.name, courseId: c.courseId, courseName: wave.COURSE_NAMES[c.courseId] || c.courseId, at: c.at, kind: 'Certificat de complétion Boyia Institute' },
  };
}
...
/* Annonces réservées aux membres connectés (ou à l'admin) : les visiteurs non
   connectés ne voient AUCUNE annonce. Alimente la cloche et le fil communauté. */
async function announcements(authHeader, adminKey) {
  const user = await userFromAuth(authHeader);
  const isAdmin = adminAuthOK(adminKey);
  if (!user && !isAdmin) return { status: 401, body: { error: 'non-connecte', announcements: [] } };
...
async function checkout(input, baseUrl) {
  ...
  /* Abonnement (Echo) : prix du plan calculé ICI (promo comprise), pas de coupon. */
  const plan = wave.SUB_PLANS[input && input.courseId];
  if (plan && wave.ECHO_FREE && plan.course === 'echo') {
    /* Période gratuite : on refuse d'encaisser un abonnement Echo. */
    return { status: 400, body: { error: 'gratuit-temporaire', message: 'Echo est actuellement offert aux membres connectés — aucun paiement nécessaire.' } };
  }
  if (plan) {
    const amount = wave.planPrice(plan);
