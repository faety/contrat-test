...
async function health() {
  return { status: 200, body: { ok: true, wave: wave.isLive(), demo: !wave.isLive(), store: store.kind, admin: !!ADMIN_PASSWORD, mail: mail.isConfigured() } };
}

/* Après le passage d'une commande en « payé » : reçu au client + notification
   interne, une seule fois (flag emailed). Ne bloque jamais le flux de paiement. */
async function afterPaid(o) {
  if (!o || o.status !== 'paid' || o.emailed || !mail.isConfigured()) return o;
  try {
...
module.exports = { health, checkout, couponPreview, adminCoupons, adminStats, webhook, order, welcome, afterPaid, readRawBody, baseUrlFrom };
